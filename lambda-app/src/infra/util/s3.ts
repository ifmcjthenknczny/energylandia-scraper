...
export async function deleteFile(locations: string[]) {
    ...
    if (S3_BUCKET) {
        ...
        for (const location of locations) {
            ...
            await fs.rm(filePath.startsWith('/') ? filePath : path.join(process.env.TEMP_PATH, filePath));
        }
    } else {
        throw new Error('S3_BUCKET not set, and TEMP_PATH not set');
    }
...
export async function downloadFile(location: string) {
    ...
    if (S3_BUCKET) {
        ...
        const filePath = location.replace('file://', '');
        return fs.readFile(filePath.startsWith('/') ? filePath : path.join(process.env.TEMP_PATH, filePath));
    } else {
        throw new Error('S3_BUCKET not set, and TEMP_PATH not set');
    }
}
